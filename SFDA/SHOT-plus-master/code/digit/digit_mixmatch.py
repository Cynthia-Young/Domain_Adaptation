...
def split_target(args):
    ...
    train_target, test_target = digit5_dataset_read(t_domain, transform, exp = False)
    train_target2, _ = digit5_dataset_read(t_domain, transform, exp = False, index= False, twice = True)
    ...
    if args.dset == 'm2u':
        ...
        new_src.train_data = np.delete(new_src.data, np.where(train_idx==0)[0], axis=0)
        new_src.train_labels = np.delete(new_src.labels, np.where(train_idx==0)[0], axis=0)

        new_tar.train_data = np.delete(new_tar.data, np.where(train_idx==1)[0], axis=0)
        new_tar.train_labels = np.delete(new_tar.labels, np.where(train_idx==1)[0], axis=0)

    else:

        new_src.data = np.delete(new_src.data, np.where(train_idx==0)[0], axis=0)
        new_src.labels = np.delete(new_src.labels, np.where(train_idx==0)[0], axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='t2m', choices=['t2m', 't2u', 't2s', 't2y', 'm2t', 'm2u', 'm2s', 'm2y', 'u2t', 'u2m', 'u2s', 'u2y', 's2t', 's2m', 's2u','s2y', 'y2t', 'y2m', 'y2u', 'y2s'], help="The dataset or source dataset used")

...
def load_mnist(base_path):
    ...
    mnist_train = mnist_train0[:25000]
    train_label = train_label0[:25000]
...
def load_mnist_m(base_path):
    ...
    mnistm_train = mnistm_train0[:25000]
    train_label = train_label0[:25000]
    ...
    # ])
    # mnistm_train = stack([transform(tensor(img)) for img in mnistm_train])
    # mnistm_test = stack([transform(tensor(img)) for img in mnistm_test])
    # mnistm_train = mnistm_train.numpy().astype(np.float32)
    # mnistm_test = mnistm_test.numpy().astype(np.float32)
...
def load_usps(base_path):
    ...
    usps_train = usps_train0[:25000]
    train_label = train_label0[:25000]
    # usps_val = usps_train0[20000:25000]
    # val_label = train_label0[20000:25000]
    # print(usps_test.shape)
    ...
    #     transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    # ])
    # usps_train = stack([transform(tensor(img)) for img in usps_train])
    # usps_test = stack([transform(tensor(img)) for img in usps_test])
    # usps_train = usps_train.numpy().astype(np.float32)
    # usps_test = usps_test.numpy().astype(np.float32)
...
def digit5_dataset_read(domain, transform, exp = False, index=False, twice=False):
    ...
    train_dataset = DigitFiveDataset(data=train_image, labels=train_label, transform=transform, index=index, twice=twice)
    test_dataset = DigitFiveDataset(data=test_image, labels=test_label, transform=transform, index=index, twice=twice)
    return train_dataset, test_dataset

# if __name__ == "__main__":
    # train_image, train_label, test_image, test_label = load_mnist('/home/schzhao/XDA/')
    # train_image, train_label, test_image, test_label = load_mnist_m('/home/schzhao/XDA/')
    # train_image, train_label, test_image, test_label = load_svhn('/home/schzhao/XDA/')
    # train_image, train_label, test_image, test_label = load_syn('/home/schzhao/XDA/')
    # train_image, train_label, test_image, test_label = load_usps('/home/schzhao/XDA/')
